...
month_cb['values'] = [month_name[m][0:10] for m in range(1, 13)]
month_cb['state'] = 'readonly'
month_cb.current(0)
month_cb.bind("<<ComboboxSelected>>",month_changed )
month_cb.place(x=400, y=100)

#   expenditure_date = datetime.date(int(values[1:5]))
...
month_cb.bind('<<ComboboxSelected>>', month_changed)
Label(text='Filter:', font=lbl_font, bg=dataentery_frame_bg).place(x=350, y=100)
...
Button(buttons_frame, text='View Selected Expense\'s Details', font=btn_font, width=25, bg=hlb_btn_bg,
      command=view_expense_details).place(x=640, y=65)
